...
<main class="contenido-principal" style="display: flex; justify-content: center; align-items: center; min-height: 100vh; width: 100%; padding: 20px;">
    <div style="width: 100%; max-width: 600px;">
        ...
        <div style="text-align: center; margin-bottom: 2rem;">
            ...
            <h2 style="color: #1a3a6b; font-weight: 700; font-size: 1.8rem;">Editar Horario</h2>
            <p style="color: #5a6b7c; font-size: 1rem;">Modifica los datos del horario</p>
        </div>

        <div class="card shadow-lg border-0" style="border-radius: 16px; overflow: hidden; width: 100%;">
            <div class="card-body p-4">
                <form method="POST" action="{{ route('admin.schedules.update', $schedule) }}" style="display: flex; flex-direction: column; align-items: center;">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3" style="width: 100%; max-width: 400px;">
                        ...
                        <select name="day" id="day" class="form-control" required style="border-radius: 8px; border: 2px solid #e9ecef; padding: 12px 16px; font-size: 1rem; width: 100%;">
                            <option value="Lunes" {{ old('day', $schedule->day) == 'Lunes' ? 'selected' : '' }}>Lunes</option>
                            <option value="Martes" {{ old('day', $schedule->day) == 'Martes' ? 'selected' : '' }}>Martes</option>
                            <option value="Miércoles" {{ old('day', $schedule->day) == 'Miércoles' ? 'selected' : '' }}>Miércoles</option>
                            <option value="Jueves" {{ old('day', $schedule->day) == 'Jueves' ? 'selected' : '' }}>Jueves</option>
                            <option value="Viernes" {{ old('day', $schedule->day) == 'Viernes' ? 'selected' : '' }}>Viernes</option>
                            <option value="Sábado" {{ old('day', $schedule->day) == 'Sábado' ? 'selected' : '' }}>Sábado</option>
                            <option value="Domingo" {{ old('day', $schedule->day) == 'Domingo' ? 'selected' : '' }}>Domingo</option>
                        </select>
                    ...
                    <div class="mb-3" style="width: 100%; max-width: 400px;">
                        ...
                        <input type="time" name="open_time" id="open_time" class="form-control" value="{{ old('open_time', $schedule->open_time ? \Carbon\Carbon::parse($schedule->open_time)->format('H:i') : '') }}" style="border-radius: 8px; border: 2px solid #e9ecef; padding: 12px 16px; font-size: 1rem; width: 100%;">
                    ...
                    <div class="mb-3" style="width: 100%; max-width: 400px;">
                        ...
                        <input type="time" name="close_time" id="close_time" class="form-control" value="{{ old('close_time', $schedule->close_time ? \Carbon\Carbon::parse($schedule->close_time)->format('H:i') : '') }}" style="border-radius: 8px; border: 2px solid #e9ecef; padding: 12px 16px; font-size: 1rem; width: 100%;">
                    ...
                    <div class="mb-4" style="width: 100%; max-width: 400px; text-align: left;">
                        <div class="form-check">
                            <input type="checkbox" name="is_closed" id="is_closed" class="form-check-input" value="1" {{ old('is_closed', $schedule->is_closed) ? 'checked' : '' }} style="border-radius: 4px; border-color: #ced4da; width: 18px; height: 18px;">
